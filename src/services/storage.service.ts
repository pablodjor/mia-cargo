...
function normalizePackage(pkg: Package): Package {
  ...
  // "Devuelto" ya no existe: se trata como no entregado.
  const status =
...
export const storageService = {
  ...
  saveSnapshot(snapshot: DatabaseSnapshot): void {
    localStorage.setItem(STORAGE_KEYS.version, String(snapshot.version))
    this.setUsers(snapshot.users)
    this.setPersons(snapshot.persons)
    this.setPackages(snapshot.packages)
    this.setDeliveries(snapshot.deliveries)
    this.setDrivers(snapshot.drivers)
    this.setVehicles(snapshot.vehicles)
    this.setCouriers(snapshot.couriers)
    this.setHistory(snapshot.history)
    this.setFailureReasons(snapshot.failureReasons)
  },
  ...
  setPackages(packages: Package[]): void {
    writeJson(STORAGE_KEYS.packages, packages.map(normalizePackage))
  },
  ...
  setDeliveries(deliveries: Delivery[]): void {
    writeJson(STORAGE_KEYS.deliveries, deliveries.map(normalizeDelivery))
  },
